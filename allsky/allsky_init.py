...
x_min: float = 0.0e0 # [m]
x_max: float = 12.0e3 # [m]

xh = np.linspace(x_min, x_max, n_xh) # Column x-interfaces; [n_xh]; [m]
x = 0.5 * (xh[1:] + xh[:-1]) # Column x-midpoints; [n_x]; [m]

n_y: int = 24
n_yh: int = n_x + 1

y_min: float = 0.0e0 # [m]
y_max: float = 12.0e3 # [m]

yh = np.linspace(y_min, y_max, n_xh) # Column y-interfaces; [n_yh]; [m]
y = 0.5 * (yh[1:] + yh[:-1]) # Column y-midpoints; [n_y]; [m]
...
z_min: float = 0.0e0 # [m]
z_max: float = 70.0e3 # [m]
z_trop: float = 15.0e3 # Nominal tropopause height; [m]

# Null grid
n_grid_x: int = max(1, n_x // 10)
n_grid_y: int = max(1, n_y // 10)
n_grid_z: int = max(1, n_z // 10)

zh = np.zeros(n_zh) # Level heights; [n_zh]; [m]
for i in range(1, n_z//2+1):
    zh[i           ] =          2.*i*(z_trop        )/n_z
    zh[i + n_z//2] = z_trop + 2.*i*(z_max - z_trop)/n_z

z = 0.5 * (zh[1:] + zh[:-1]) # Layer heights; [n_z]; [m]
...
#-------------------------------------------------------------------------------
# Write rte_rrtmgp_input.nc
#-------------------------------------------------------------------------------
nc_file.createDimension("x", n_x)
nc_file.createDimension("xh", n_xh)
nc_file.createDimension("y", n_y)
nc_file.createDimension("yh", n_yh)
nc_file.createDimension("z", n_z)
nc_file.createDimension("zh", n_zh)
nc_file.createDimension("lay", n_z)
nc_file.createDimension("lev", n_zh)
...
# Null Grid Information
nc_ngrid_x = nc_file.createVariable("ngrid_x", int_type)
nc_ngrid_x[:] = n_grid_x

nc_ngrid_y = nc_file.createVariable("ngrid_y", int_type)
nc_ngrid_y[:] = n_grid_y

nc_ngrid_z = nc_file.createVariable("ngrid_z", int_type)
nc_ngrid_z[:] = n_grid_z

# Pressure, Temperature Information
nc_p_lay = nc_file.createVariable("p_lay", float_type, ("lay", "y", "x"))
nc_p_lay[:,:,:] = np.tile(p_lay[:,None,None], (1, n_y, n_x))

nc_p_lev = nc_file.createVariable("p_lev", float_type, ("lev", "y", "x"))
nc_p_lev[:,:,:] = np.tile(p_lev[:,None,None], (1, n_y, n_x))

nc_T_lay = nc_file.createVariable("t_lay", float_type, ("lay", "y", "x"))
nc_T_lay[:,:,:] = np.tile(T_lay[:,None,None], (1, n_y, n_x))

nc_T_lev = nc_file.createVariable("t_lev", float_type, ("lev", "y", "x"))
nc_T_lev[:,:,:] = np.tile(T_lev[:,None,None], (1, n_y, n_x))

# Gas Volume Mixing Ratio Information
...
nc_CH4[:] = ch4
nc_N2O[:] = n2o
nc_O3 [:,:,:] = np.tile(o3 [:,None, None], (1, n_y, n_x))
nc_H2O[:,:,:] = np.tile(h2o[:,None, None], (1, n_y, n_x))
nc_N2 [:] = n2
nc_O2 [:] = o2
...
sfc_alb_dir = np.ones((n_y, n_x, n_bnd_sw))*0.06
sfc_alb_dif = np.ones((n_y, n_x, n_bnd_sw))*0.06
...
cloud_flag = (np.arange(1, n_x+1)%3 > 0)
