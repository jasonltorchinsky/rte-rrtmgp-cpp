...
def main():
    ...
    ngrid_y: int = max(24, np.ceil(n_col_y / 10))
    ...
    eff_radius_qc: np.ma.MaskedArray = np.squeeze(nc_input.variables["eff_radius_qc"][:].astype(np_float), axis = 0) # Effective raidus of cloud rain particles [μm]; (ncol, n_lay_z)
    ...
    rel[rel < 2.5] = 2.5 #< eff_radius_qc.min()] = eff_radius_qc.min()
    rel[rel > 21.5] = 21.5 #> eff_radius_qc.max()] = eff_radius_qc.max()
    ...
    solar_zenith_angle: np.float64 = np.deg2rad(45.00) # [N/A]
