...
def write_single_timestep(src_path: Path, t_index: int, dst_path: Path) -> None:
    ...
    try:
        ...
        ds_t = ds.isel(time=t_index, drop=True)
...
def load_missing_output_requests(path: Path) -> set[tuple[str, int]]:
    """
    Read a file containing missing output filenames or full paths.
    Returns a set of (lr_tag, t_index), e.g. {("lr_04", 146), ...}
    """
    ...
    with path.open() as f:
        for raw in f:
            ...
            base = Path(line).name
            lr_tag, _ = parse_lr_tag(base)
            t_index = parse_t_index(base)

            if t_index is None:
                raise ValueError(f"Could not parse t_index from line: {line}")
            if lr_tag == "lr_00":
                raise ValueError(f"Could not parse lr_tag from line: {line}")

            requests.add((lr_tag, t_index))
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional text file listing missing output filenames/paths. "
             "If provided, only tasks corresponding to those missing outputs are queued.",
    )
    args = ap.parse_args()
    ...
    missing_requests: set[tuple[str, int]] | None = None
    if args.missing_output_list is not None:
        if not args.missing_output_list.exists():
            raise FileNotFoundError(f"Missing output list not found: {args.missing_output_list}")
        missing_requests = load_missing_output_requests(args.missing_output_list)
        log(f"Loaded {len(missing_requests)} missing output requests from {args.missing_output_list}")

        available_lr_tags = {parse_lr_tag(f.name)[0] for f in inputs}
        requested_lr_tags = {lr for lr, _ in missing_requests}
        unmatched = requested_lr_tags - available_lr_tags
        if unmatched:
            raise RuntimeError(
                f"Missing-output list requested lr tags not present in inputs: {sorted(unmatched)}"
            )
    ...
    for f in inputs:
        ...
        for t in range(nt):
            if missing_requests is not None and (lr_tag, t) not in missing_requests:
                continue
            q.put(Task(str(f), t, nt))
            total_tasks += 1
            queued_for_file += 1

        log(f"Queued {queued_for_file} tasks from {f.name}")

    if total_tasks == 0:
        raise RuntimeError("No tasks were queued. Check --missing-output-list and --lr filtering.")
