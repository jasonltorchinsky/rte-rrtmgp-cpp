...
def calc_sw_heating(dp_scream_file: str, sort_mask: NP_ARRAY[NP_INT], time_indices: NP_ARRAY[NP_INT],
    ...
    if method == "pdel":
        ...
        if x_indices is None:
            ...
            sw_flux_up_abs: XR_DATAARRAY = (sw_flux_up.diff("ilev")).rename({"ilev" : "lev"}).assign_coords({"lev" : qc["lev"]}) # Absorbed upwelling shortwave fluxes; [time, lev, y, x]; [W m^{-2}]
            sw_flux_dn_abs: XR_DATAARRAY = (-sw_flux_dn.diff("ilev")).rename({"ilev" : "lev"}).assign_coords({"lev" : qc["lev"]}) # Absorbed downwelling shortwave fluxes; [time, lev, y, x]; [W m^{-2}]
            ...
            sw_heating: XR_DATAARRAY = ((sw_flux_abs * vol_cell) / (cp_cell * mass_cell)) * sec_per_day # [time, lev, y, x]; [K d^{-1}]
            sw_heating = (sw_heating
                .assign_attrs({"units" : "K d^{-1}", 
                               "long_name" : "shortwave heating rate",
                               "standard_name" : "shortwave_heating_rate"})
            ...
            for ii in range(0, 3): # Assume Morning-Noon-Night indices
                ...
                sw_flux_up_abs_x: XR_DATAARRAY = (sw_flux_up_x.diff("ilev")).rename({"ilev" : "lev"}).assign_coords({"lev" : qc["lev"]}) # Absorbed upwelling shortwave fluxes; [lev, y]; [W m^{-2}]
                sw_flux_dn_abs_x: XR_DATAARRAY = (-sw_flux_dn_x.diff("ilev")).rename({"ilev" : "lev"}).assign_coords({"lev" : qc["lev"]}) # Absorbed downwelling shortwave fluxes; [lev, y]; [W m^{-2}]

                dz_x: NP_ARRAY[NP_REAL] = dz.isel(time = ii, x = x_indices[ii]).to_numpy().astype(NP_REAL) # Level geometric thickness; [lev, y]; [m]
                sw_flux_abs_x: NP_ARRAY[NP_REAL] = (sw_flux_up_abs_x + sw_flux_dn_abs_x).to_numpy().astype(NP_REAL) / dz_x # Absorbed shortwave fluxes; [lev, y]; [W m^{-3}]
                ...
                sw_heating[ii] = ((sw_flux_abs_x * vol_cell_x) / (cp_cell_x * mass_cell_x)) * sec_per_day # [lev, y]; [K d^{-1}]
