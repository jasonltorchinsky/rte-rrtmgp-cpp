...
def main():
    ...
    for ii in range(0, ndays):
        ...
        #-----------------------------------------------------------------------
        current_time: str = datetime.now().strftime("%H:%M:%S")
        msg: str = "[{}]: Calculating shortwave heating rates for day {} of {}...".format(current_time, ii, ndays - 1)
        print(msg, flush = True)

        sw_heating_via_pdel: list[NP_ARRAY[NP_REAL]] = calc_sw_heating(dp_scream_file, sort_mask, 
            mnn_indices[ii], x_indices, method = "pdel") # Shortwave heating rate, calculated via pdel; [K d^{-1}]; 3 * [lev, y]

        sw_heating_via_flux: list[NP_ARRAY[NP_REAL]] = calc_sw_heating(dp_scream_file, sort_mask, 
            mnn_indices[ii], x_indices, method = "flux") # Shortwave heating rate, calculated via flux; [K d^{-1}]; 3 * [lev, y]

        current_time: str = datetime.now().strftime("%H:%M:%S")
        msg: str = "[{}]: Calculating shortwave absorbed flux for day {} of {}...".format(current_time, ii, ndays - 1)
        print(msg, flush = True)

        sw_flux_abs_via_pdel: list[NP_ARRAY[NP_REAL]] = calc_sw_flux_abs(dp_scream_file, sort_mask, 
            mnn_indices[ii], x_indices, method = "pdel") # Shortwave absorbed flux, calculated via pdel; [W m^{-3}]; 3 * [lev, y]

        sw_flux_abs_via_flux: list[NP_ARRAY[NP_REAL]] = calc_sw_flux_abs(dp_scream_file, sort_mask, 
            mnn_indices[ii], x_indices, method = "flux") # Shortwave absorbed flux, calculated via flux; [W m^{-3}]; 3 * [lev, y]
        ...
        sw_flux_abs_via_flux = [sw_flux_abs_via_flux[ll][...,y_bound_indices[ll][0]:y_bound_indices[ll][1]] for ll in range(0, 3)]
        ...
        sw_flux_abs_max: list[NP_REAL] = [max(sw_flux_abs_via_pdel[ll].max(), sw_flux_abs_via_flux[ll].max()) for ll in range(0, 3)]
        sw_flux_abs_min: list[NP_REAL] = [min(sw_flux_abs_via_pdel[ll].min(), sw_flux_abs_via_flux[ll].min()) for ll in range(0, 3)]
        ...
        sw_heating_via_pdel_pcm: list[MPL_PCOLORMESH] = [[] for _ in range(0, ncols)]
        ll: int
        for ll in range(0, ncols):
            sw_heating_via_pdel_pcm[ll] = axs[1, ll].pcolormesh(y_grid[ll], z_grid[ll], sw_heating_via_pdel[ll],
                norm = colors.LogNorm(vmin = sw_heating_min[ll], vmax = sw_heating_max[ll]),
                cmap = heating_cmap)

        # Row 2: Shortwave Heating via flux
        sw_heating_via_flux_pcm: list[MPL_PCOLORMESH] = [[] for _ in range(0, ncols)]
        ll: int
        for ll in range(0, ncols):
            sw_heating_via_flux_pcm[ll] = axs[2, ll].pcolormesh(y_grid[ll], z_grid[ll], sw_heating_via_flux[ll],
                norm = colors.LogNorm(vmin = sw_heating_min[ll], vmax = sw_heating_max[ll]),
                cmap = heating_cmap)

        # Row 3: Shortwave absorbed flux via pdel
        sw_flux_abs_via_pdel_pcm: list[MPL_PCOLORMESH] = [[] for _ in range(0, ncols)]
        ll: int
        for ll in range(0, ncols):
            sw_flux_abs_via_pdel_pcm[ll] = axs[3, ll].pcolormesh(y_grid[ll], z_grid[ll], sw_flux_abs_via_pdel[ll],
                norm = colors.LogNorm(vmin = sw_flux_abs_min[ll], vmax = sw_flux_abs_max[ll]),
                cmap = flux_cmap)

        # Row 4: Shortwave absorbed flux via flux
        sw_flux_abs_via_flux_pcm: list[MPL_PCOLORMESH] = [[] for _ in range(0, ncols)]
        ll: int
        for ll in range(0, ncols):
            sw_flux_abs_via_flux_pcm[ll] = axs[4, ll].pcolormesh(y_grid[ll], z_grid[ll], sw_flux_abs_via_flux[ll],
                norm = colors.LogNorm(vmin = sw_flux_abs_min[ll], vmax = sw_flux_abs_max[ll]),
                cmap = flux_cmap)
        ...
        axs[1,0].set_ylabel(r"Shortwave Heating via $pdel$")
        axs[2,0].set_ylabel(r"Shortwave Heating via $flux$")
        ...
        #-----------------------------------------------------------------------
        plt_filename = "dpscream_rad_tran_day_{}.png".format(ii)
