...
def main(argv):
    ...
    # Different plots
    field_key: str
    for field_str in ["sza", "clt_max", "clt_ran", "clt_min"]:
        if field_str == "sza":
            # Solar zenith angle
            field_key = "cosine_solar_zenith_angle"
            field: XR_DATAARRAY = xr_dpscream[field_key].isel(ncol = 0)
            fields: XR_DATAARRAY = np.acos(cosine_solar_zenith_angle)

            ylabel: str = "Solar Zenith Angle"

        elif field_str in ["clt_max", "clt_ran", "clt_min"]:
            field_key = "cldfrac_tot"
    
            field: XR_DATAARRAY = xr_dpscream[field_key].mean(dim = "ncol")
    ...
    ax.plot(t, field.values.astype(NP_REAL, copy = False))

    ax.set_title(file_name_root)
    ax.set_xlabel("Time Step")
    ax.set_ylabel(ylabel)

    visualization_file_name: str = file_name_root + "_" + field_key + ".png"
