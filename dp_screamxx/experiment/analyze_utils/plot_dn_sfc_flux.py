    ...
    for lr_tag, (in_path, out_path) in pairs.items():
        ...
        with xr.open_dataset(in_path, engine="netcdf4", decode_times=False) as ds_in:
            if rank == 0:
                ...
                profile_label = time_in = x = y = mu0 = None
        ...
        with xr.open_dataset(out_path, engine="netcdf4", decode_times=False) as ds_out:
            ...
            ts_sfc_dn = (
                ds_out["sw_flux_dn"].isel(lev=0, x=slice(i0, i1)).astype("float64").load().values
            )
            ...
            for t_idx in t_indices:
                ...
                ray_xy = rt_sfc_dn[t_idx]
                ts_xy = ts_sfc_dn[t_idx]
        ...
        if rank == 0:
            ...
            pcm_cloud = pcm_dn = pcm_diff = None
            ...
            cbar_dn = fig.colorbar(
                pcm_dn, ax=axes[1:3, :], location="right", shrink=0.95, fraction=0.046, pad=0.04
            )
            cbar_dn.set_label(r"Downwelling Shortwave Surface Flux $\left[W\,m^{-2}\right]$")

            cbar_diff = fig.colorbar(
                pcm_diff, ax=axes[3, :], location="right", shrink=0.95, fraction=0.046, pad=0.04
            )
            cbar_diff.set_label(r"Downwelling Shortwave Surface Flux Difference $\left[W\,m^{-2}\right]$")

            fig.suptitle(f"Horizontal Resolution - {profile_label}", fontsize=14)

            out_png = os.path.join(viz_dir, f"{lr_tag}_dn_sfc_flux.png")
