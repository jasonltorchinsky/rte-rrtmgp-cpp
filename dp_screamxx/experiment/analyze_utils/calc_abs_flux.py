...
def calc_abs_flux(rad_tran_outfile, out_time_index, y_index = slice(0, None), zmax_index = None):
    ...
    ts_flux_dn = rad_tran_outds["sw_flux_dn"].isel(time = out_time_index, lev = slice(0, levmax_index), y = y_index) # [W m^{-2}] [time, lev, x]
    ts_flux_up = rad_tran_outds["sw_flux_up"].isel(time = out_time_index, lev = slice(0, levmax_index), y = y_index) # [W m^{-2}] [time, lev, x]
    lev = rad_tran_outds["lev"].isel(lev = [0, 1]).values # [m]

    rt_flux_abs_dif = rad_tran_outds["rt_flux_abs_dif"].isel(time = out_time_index, z = slice(0, laymax_index), y = y_index) # [W m^{-3}] [time, lay, x]
    rt_flux_abs_dir = rad_tran_outds["rt_flux_abs_dir"].isel(time = out_time_index, z = slice(0, laymax_index), y = y_index) # [W m^{-3}] [time, lay, x]
