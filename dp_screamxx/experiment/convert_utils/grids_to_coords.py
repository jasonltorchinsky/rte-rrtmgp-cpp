...
def grids_to_coords(g_grids_tgt: dict, time: NP_ARRAY[NP_REAL], coarse_factors: NP_ARRAY[NP_REAL], comm: MPI_COMM) -> Optional[dict]:
    ...
    if l_rank == MPI_ROOT:
        ...
        for coarse_factor in coarse_factors:
            ...
            #-------------------------------------------------------------------
            x: NP_ARRAY[NP_REAL] = g_grids_tgt[coarse_str]["x"]
            dx: NP_REAL = x[1] - x[0]
            xh: NP_ARRAY[NP_REAL] = np.concatenate([x - dx / 2., [x[-1] + dx / 2.]]) # Column interfaces

            y: NP_ARRAY[NP_REAL] = g_grids_tgt[coarse_str]["y"]
            dy: NP_REAL = y[1] - y[0]
            ...
            g_coords[coarse_str]: dict = dict(
                ...
                z = ("z", g_grids_tgt[coarse_str]["z_lay"], dict(description = grid_descriptions["z"], units = grid_units["z"])),
                zh = ("zh", g_grids_tgt[coarse_str]["z_lev"], dict(description = grid_descriptions["zh"], units = grid_units["zh"])),
                z_lay = ("z_lay", g_grids_tgt[coarse_str]["z_lay"], dict(description = grid_descriptions["z_lay"], units = grid_units["z_lay"])),
                z_lev = ("z_lev", g_grids_tgt[coarse_str]["z_lev"], dict(description = grid_descriptions["z_lev"], units = grid_units["z_lev"])),
            )
