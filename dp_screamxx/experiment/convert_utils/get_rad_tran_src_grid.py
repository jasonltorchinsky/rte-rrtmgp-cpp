...
def get_rad_tran_src_grid(dp_scream_file: str, l_time_idxs: NP_ARRAY[NP_INT], comm: MPI_COMM) -> dict:
    ...
    with xr.open_dataset(dp_scream_file, engine = "netcdf4", decode_timedelta = False) as xr_dp_scream:
        l_z_int: XR_DATAARRAY = xr_dp_scream["z_int"].isel(time = l_time_idxs) # Geometric height at level interfaces [top -> bot]; [l_time, ncol, ilev]; [m]
    l_z_bot: NP_REAL = NP_REAL(l_z_int.isel(ilev = -1).max()) # [m]
    l_z_top: NP_REAL = NP_REAL(l_z_int.isel(ilev = 0).min()) # [m]
    
    g_z_bot: NP_REAL = comm.allreduce(l_z_bot, op = MPI.MAX) # [m]
    g_z_top: NP_REAL = comm.allreduce(l_z_top, op = MPI.MIN) # [m]
    ...
    xr_dp_scream: XR_DATASET
    with xr.open_dataset(dp_scream_file, engine = "netcdf4", decode_timedelta = False) as xr_dp_scream:
        lon: XR_DATAARRAY = xr_dp_scream["lon"] # x-position at column midpoints; [ncol]; [m]
        lat: XR_DATAARRAY = xr_dp_scream["lat"] # y-position at column midpoints; [ncol]; [m]
    x: NP_ARRAY[NP_REAL] = np.unique(lon).astype(NP_REAL) # [n_x]; [m]
    y: NP_ARRAY[NP_REAL] = np.unique(lat).astype(NP_REAL) # [n_y]; [m]
